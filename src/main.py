...
if __name__ == "__main__":
    ...
    for new in news:
        ...
        context.add_message("user", f"{new['Statement']}"
                                    ". Give me google search queries you want to search to fact check this news. Give me "
                                    "the queries in json format, where the key is 'queries' and the value is a list of "
        ...
        with engine.connect() as connection:
            ...
            connection.execute(
                f"UPDATE news SET model_label = '{judgement}', model_explaination = '{escaped_explanation}', model_response='{escaped_content}' WHERE id = {new['id']}")
